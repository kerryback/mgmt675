...
\title{Module 5: Discounted Cash Flow Analysis}
\author{Kerry Back}
\date{}
...
\begin{frame}[shrink=10]{Simulation Setup}
...
\node[sbox] (dist) {Random\\Assumptions};
\node[sbox, right=0.6cm of dist] (proforma) {Pro Forma\\Model};
\node[sbox, right=0.6cm of proforma] (fcf) {FCF};
\node[sbox, right=0.6cm of fcf] (ev) {Enterprise\\Value};
\node[sbox, right=0.6cm of ev, fill=alertorange!15] (hist) {Histogram\\of Values};

\draw[sarrow] (dist) -- (proforma);
\draw[sarrow] (proforma) -- (fcf);
\draw[sarrow] (fcf) -- (ev);
\draw[sarrow] (ev) -- (hist);

\draw[->, thick, accentblue, rounded corners] (ev.south) -- ++(0,-0.5) -| (dist.south) node[midway, below, font=\scriptsize\itshape, text=titlegray] {repeat 10{,}000$\times$};
\end{tikzpicture}
\end{center}
\vspace{0.1cm}
\begin{center}
...
\begin{frame}{In-Class Exercise: Pro Forma Skill}
Create a Claude skill that enforces specific pro forma forecasting rules for PP\&E:
\vspace{0.3cm}
\begin{shadedbox}[title=\textbf{Rules to Encode in the Skill}]
\begin{enumerate}\small
  \item Forecast \textbf{net PP\&E} as a percentage of sales (e.g., trailing average of net PP\&E / sales)
  \item Forecast \textbf{depreciation} as a percentage of net PP\&E (e.g., trailing average of depreciation / net PP\&E)
  \item Compute \textbf{cap ex} as the plug: $\text{cap ex} = \Delta\,\text{net PP\&E} + \text{depreciation}$
\end{enumerate}
\end{shadedbox}
\vspace{0.3cm}
\begin{enumerate}\small
  \item Write a \texttt{SKILL.md} that tells Claude to follow these three rules whenever building pro formas
  \item Test it: ask Claude to build a pro forma for a company using the skill
  \item Verify: does the balance sheet balance?  Is cap ex the plug?
\end{enumerate}
\end{frame}
