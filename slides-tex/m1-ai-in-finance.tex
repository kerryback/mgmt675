...
\vfill
\centerline{\scriptsize CNBC interview, September 30, 2025.}
\end{frame}
...
\vspace{0.1cm}
{\scriptsize ``How HPE's CFO used AI to transform the 100-slide Monday meeting her team spent all week preparing for,'' \textit{Fortune}, Feb.\ 12, 2026.}
\end{frame}
...
Every agent follows a loop: \textbf{plan, execute, observe, iterate.}
\vspace{0.3cm}
\begin{enumerate}
  \item \textbf{Plan} --- the LLM reads your request and breaks it into steps
  \item \textbf{Execute} --- the agent calls a tool (runs code, queries a database, writes a file)
  \item \textbf{Observe} --- the agent checks the result of the action
  \item \textbf{Iterate} --- if the result is not satisfactory, the agent adjusts its plan and tries again
\end{enumerate}
\vspace{0.3cm}
When an agent encounters an error, it does not stop and ask you what to do.  It diagnoses the problem, revises its approach, and tries again.  This \alert{autonomous problem-solving} is what separates agents from simple automation scripts.
\end{frame}

\begin{frame}{How Tool Calls Work}
The LLM does not run code or call databases directly.  When it decides a tool is needed, it returns a structured message:
\vspace{0.2cm}
\begin{shadedbox}
\small\texttt{\{``tool'': ``sql\_query'', ``query'': ``SELECT \ldots''\}}
\end{shadedbox}
\vspace{0.2cm}
...
\vspace{0.3cm}
An agent can use \alert{multiple tools in sequence}.  For example: query a database, pass the results to Python for analysis, generate a chart, and assemble everything into a PowerPoint deck --- all from a single instruction.
\end{frame}
...
\end{itemize}
\vspace{0.3cm}
You type a question (\textit{``Show me AAPL closing prices for 2024''}).  The agent sends it to ChatGPT, which returns a SQL query.  The agent runs the query against the database and displays the answer.  If the query fails, ChatGPT rewrites it and tries again --- multiple rounds, no human intervention.
\end{frame}
