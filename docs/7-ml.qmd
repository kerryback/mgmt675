...
- Overfitting means that the parameters are chosen based on chance relationships in the training data.
...
- Increasing complexity makes underfitting less likely but makes overfitting more likely.  
...
- The penalty in lasso and ridge is called alpha in scikit-learn.  You can specify it, but there is a default value that Julius will probably use.
...
- The penalty controls the degree of complexity - higher penalty = less complex.
...
- Ask Julius to cross-validate lasso regression.
- You can (and may need to) specify a set of alpha values to try.
