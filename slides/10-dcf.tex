...
\begin{frame}{Goals}
\begin{baritemize}
  \item Build \textbf{pro forma financial statements} from a small set of assumptions
  \item Compute \textbf{free cash flow to the firm} from the pro formas
  \item Perform a \textbf{two-stage DCF valuation}
  \item Conduct \textbf{sensitivity analysis} on key inputs
  \item Use \textbf{Monte Carlo simulation} to compute expected cash flows and a distribution of values
  \item Do all of this using AI across multiple platforms
\end{baritemize}
\end{frame}
...
\vspace{0.1cm}
\begin{shadedbox}
\textbf{Net other operating assets} = non-current operating assets (goodwill, intangibles, etc.) minus non-current operating liabilities (excluding debt and deferred taxes).
\end{shadedbox}
\end{frame}
...
\end{columns}
\vspace{0.3cm}
\begin{shadedbox}[title=\textbf{Free Cash Flow to the Firm}]
\begin{center}
...
\begin{baritemize}
  ...
  \item Discount all cash flows at the \textbf{WACC}
  \item \textbf{Equity value} = Enterprise value $-$ net debt
\end{baritemize}
\end{frame}
...
\begin{baritemize}
  \item Sensitivity tables vary one or two inputs at a time
  \item In reality, \alert{all assumptions are uncertain simultaneously}
...
\begin{baritemize}
  ...
  \item Plot a histogram of the value distribution
  \item Identify which assumptions drive the most variance
\end{baritemize}
\end{frame}
