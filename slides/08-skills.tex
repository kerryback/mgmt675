...
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\begin{shadedbox}[title=\textbf{SKILL.md (Sketch)}]\scriptsize
\begin{verbatim}
---
name: tasks
...
\end{shadedbox}
\vspace{0.1cm}
\begin{shadedbox}[title=\textbf{Why This Works}]
\begin{itemize}\scriptsize
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{A Plugin Can Include}]
\begin{itemize}\small
  \item Skills (\texttt{SKILL.md} files)
  \item Agents (\texttt{AGENT.md} files)
  \item Hooks (auto-run on events)
  \item MCP servers (external tools)
  \item LSP servers (language analysis)
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Why Use Plugins?}]
\begin{itemize}\small
...
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\begin{shadedbox}[title=\textbf{Directory Structure}]\small
\begin{verbatim}
my-plugin/
  .claude-plugin/
...
\end{column}
\begin{column}{0.55\textwidth}
\begin{shadedbox}[title=\textbf{plugin.json (Required)}]\small
\begin{verbatim}
{
  "name": "my-plugin",
...
\begin{itemize}\small
  ...
  \item Private / enterprise registries
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Example Plugin Categories}]
\begin{itemize}\small
  \item Code review and linting
  \item Git workflow automation
...
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Resources}]
\begin{itemize}\small
  \item \href{https://code.claude.com/docs/en/plugins.md}{Plugins documentation}
  \item \href{https://code.claude.com/docs/en/discover-plugins.md}{Discover \& install plugins}
  \item \href{https://code.claude.com/docs/en/plugin-marketplaces.md}{Plugin marketplaces}
  \item \href{https://code.claude.com/docs/en/skills.md}{Skills documentation}
...
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\begin{shadedbox}[title=\textbf{Plugin Structure}]\scriptsize
\begin{verbatim}
finance/
  .claude-plugin/
    plugin.json
...
\end{column}
\begin{column}{0.55\textwidth}
\begin{shadedbox}[title=\textbf{Slash Commands}]
\begin{itemize}\scriptsize
  ...
  \item \texttt{/finance:income-statement} --- period-over-period comparison
  \item \texttt{/finance:variance-analysis} --- decompose budget vs.\ actual gaps
  \item \texttt{/finance:sox-testing} --- SOX compliance workpapers
\end{itemize}
\end{shadedbox}
\vspace{0.1cm}
\begin{shadedbox}[title=\textbf{Data Connectors (MCP)}]
\begin{itemize}\scriptsize
...
\begin{itemize}\small
  ...
  \item Required for SOX internal controls
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Key Decompositions}]
\begin{itemize}\small
  ...
  \item \textbf{Operating income}: revenue variance + cost variance
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Revenue Variance: $-$\$155K}]
\begin{itemize}\scriptsize
...
\end{shadedbox}
\vspace{0.1cm}
\begin{shadedbox}[title=\textbf{COGS Variance: $-$\$40K}]
\begin{itemize}\scriptsize
...
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{SG\&A Variance: $-$\$105K}]
\begin{itemize}\scriptsize
...
\end{shadedbox}
\vspace{0.1cm}
\begin{shadedbox}[title=\textbf{Operating Income Bridge}]
\begin{itemize}\scriptsize
...
\end{shadedbox}
\vspace{0.2cm}
\begin{shadedbox}[title=\textbf{Prompt}]
...
\end{shadedbox}
\vspace{0.2cm}
\begin{shadedbox}[title=\textbf{What the Plugin Does}]
\begin{enumerate}\small
  \item \textbf{Skill activates}: domain knowledge about variance decomposition formulas
  \item \textbf{MCP connector} pulls data (or reads the uploaded spreadsheet)
  \item \textbf{Decomposes} revenue into volume + price; COGS into volume + rate
  \item \textbf{Generates} waterfall chart (budget $\rightarrow$ adjustments $\rightarrow$ actual)
...
\begin{itemize}\scriptsize
  ...
  \item Legal plugin: contract review, NDA triage, compliance checks
  \item Finance plugin: close management, reconciliation, variance analysis
...
\end{shadedbox}
\vspace{0.1cm}
\begin{shadedbox}[title=\textbf{The Counterargument}]
\begin{itemize}\scriptsize
