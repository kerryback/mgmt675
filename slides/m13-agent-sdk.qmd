...
:::{.explainer}
The `account_id` tells the agent [whose]{.amber} portfolio to look up. In a multi-client system, this is how you scope data access to the right account.
:::
...
def execute_tool(name: str, inputs: dict) -> str:
    ...
    if name == "get_holdings":
        ...
        return json.dumps(HOLDINGS)  # mock: same data for any account
...
Our portfolio agent is deployed on Railway:

**[web-production-9f0f9.up.railway.app](https://web-production-9f0f9.up.railway.app)**

:::{.explainer}
Same code we just walked through — the only additions are a small web server (Flask/FastAPI) to handle HTTP requests and a `requirements.txt` for Railway to install dependencies.
:::
