...
\vspace{0.2cm}
We start with the cloud sandbox today.  Modules 3--4 introduce local execution and data connections.
\end{frame}

% ========================================
% RICE DATA PORTAL
% ========================================
\section{Rice Data Portal}

\begin{frame}{Rice Data Portal: Stock Market Database}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{What It Is}]
\begin{itemize}\small
  \item Stock market database for Rice students
  \item 59{,}000+ companies, 17M+ stock prices
  \item Financial statements from SEC filings
  \item Guide: \url{https://portal-guide.rice-business.org}
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Using the Web Interface}]
\begin{itemize}\small
  \item Visit \href{https://data-portal.rice-business.org}{data-portal.rice-business.org} and sign in with Rice credentials
  \item Ask questions in plain English---the portal uses ChatGPT to generate SQL and query the database
...
\begin{itemize}
  \item \textbf{SEP} (daily prices): \texttt{ticker}, \texttt{date}, \texttt{close} (split-adjusted), \texttt{closeadj} (fully adjusted), \texttt{volume}
  \item \textbf{DAILY} (valuation): \texttt{ticker}, \texttt{date}, \texttt{marketcap}, \texttt{pb}, \texttt{pe}, \texttt{ps}
  \item \textbf{TICKERS} (company info): \texttt{ticker}, \texttt{name}, \texttt{sector}, \texttt{industry}
  \item \textbf{SF1} (fundamentals): \texttt{revenue}, \texttt{netinc}, \texttt{assets}, \texttt{equity}, \texttt{debt}, \texttt{roe}, \ldots
\end{itemize}
\vfill
\textbf{How to use it:} Describe the data you need in the portal's web interface.  It generates and runs the SQL query, then lets you download the results.  In Module 4, we'll automate this with Claude Code.
\end{frame}

\begin{frame}{Computing Returns}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Monthly Returns from \texttt{closeadj}}]
\begin{itemize}\small
  \item $r_t = \texttt{closeadj}_t \;/\; \texttt{closeadj}_{t-1} - 1$
  \item Use \texttt{closeadj}, not \texttt{close}: it accounts for dividends and spinoffs, giving \alert{total return}
  \item \texttt{close} (split-adjusted only) is useful for charting prices and moving averages
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Important}]
\begin{itemize}\small
  \item Always \alert{group by ticker} when computing returns---never mix prices across stocks
  \item For monthly data, use end-of-month prices
  \item Example prompt for the portal: ``Get end-of-month closeadj for SPY, AGG, and GLD from 2019 to 2024''
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{enumerate}
  ...
  \item Data Exploration \& Strategic Thinking
...
\begin{enumerate}
  ...
  \item Verification and Governance
...
\textbf{Claude}
\begin{itemize}\small
  \item \textbf{Free:} Sonnet 4.5 chat, no code execution or coding agent
  \item \textbf{Pro (\$20/mo):} Sonnet 4.5 + limited Opus 4.6, Cowork (cloud VM), and Claude Code
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\textbf{ChatGPT}
\begin{itemize}\small
  \item \textbf{Free:} GPT-5 (limited) with Data Analysis
  \item \textbf{Plus (\$20/mo):} GPT-5.2 with Data Analysis and Codex (GPT-5.3)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
