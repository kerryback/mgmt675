...
\title{Module 8: Connecting AI to Data and Tools}
\author{Kerry Back}
\date{}
...
% ========================================
% SECTION 1: MCP — CONNECTING AI TO TOOLS
% ========================================
\section{MCP: Connecting AI to Tools}

\begin{frame}{Skills vs.\ MCP Servers}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Skill = Text}]
\begin{itemize}\small
  \item A markdown file with instructions
  \item Shapes how the LLM \textit{thinks} and \textit{responds}
  \item No code runs; just prompt text
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{MCP Server = Running Program}]
\begin{itemize}\small
  \item A small program that stands between the AI and an external tool
  \item Makes API calls, queries databases, controls browsers
  \item Returns structured results to the LLM
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
\vspace{0.3cm}
Skills tell the LLM \textit{what to say}.  MCP servers let the LLM \textit{take actions} in the outside world.
\end{frame}
...
\node[sbox] (user) {User};
\node[sbox, right=1.8cm of user, fill=accentblue!15] (agent) {Agent};
\node[sbox, above right=0.8cm and 2cm of agent] (llm) {LLM};
\node[sbox, below right=0.8cm and 2cm of agent, fill=alertorange!15] (mcp) {MCP\\Server};
\node[sbox, right=1.8cm of mcp] (tool) {External\\Tool};
...
\draw[sarrow] ([yshift=2pt]agent.east) -- ([yshift=2pt]mcp.west);
\draw[sarrow] ([yshift=-2pt]mcp.west) -- ([yshift=-2pt]agent.east);
...
\node[right=0.4cm of s2, font=\scriptsize, text=titlegray] {Alpha Vantage, FMP, \ldots};
...
% ========================================
% SECTION 5: COMBINING CONNECTIONS
% ========================================
