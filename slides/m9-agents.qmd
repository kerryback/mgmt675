...
[1]{.step-icon}
[Sign Up]{.step-title}
[Create an OpenRouter account]{.step-desc}
:::
:::{.step-card}
[2]{.step-icon}
[Chat]{.step-title}
[Talk to a model in the browser]{.step-desc}
:::
:::{.step-card}
[3]{.step-icon}
[API Key]{.step-title}
[Get your key and save it]{.step-desc}
:::
:::{.step-card}
[4]{.step-icon}
[Code]{.step-title}
[Claude writes a script that calls a model]{.step-desc}
:::
:::{.step-card}
[5]{.step-icon}
[App]{.step-title}
[Claude builds a chatbot web app]{.step-desc}
:::
:::{.step-card}
[6]{.step-icon}
[Customize]{.step-title}
[Add a system prompt]{.step-desc}
:::
:::

:::{.explainer style="text-align:center; margin-top:1em;"}
Each step builds on the last. By step 6 you will have a [working chatbot app]{.amber} with a custom personality.
:::
...
:::{.info-box}
*Install the openai package. Then write a Python script that sends a message to <model name> via OpenRouter using my API key (which is in the environment variable OPENROUTER_API_KEY). Show me the code, then run it.*
:::
...
My chatbot on Railway: https://pirate-chatbot-productio
  n.up.railway.app
...
:::{.explainer style="text-align:center;"}
My mean-variance app on Railway: https://chatbot-app-production-0
  fb1.up.railway.app
:::
