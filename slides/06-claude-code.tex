...
\title{Claude Desktop in Code Mode}
\author{Kerry Back}
\date{}
...
\begin{baritemize}
\item Download exercise2\_invoices.zip into your project folder.
\item Extract the files.  You will have an invoice register (PDF), a payment log (xlsx), and a vendor disputes memo (docx).
\item In Claude Desktop, go to Code $\rightarrow$ Local and select your project folder.
\item Ask Claude to extract the invoice table from the PDF, match invoices to payments (handling inconsistent reference formats), and identify which invoices are fully paid, partially paid, or unpaid.
