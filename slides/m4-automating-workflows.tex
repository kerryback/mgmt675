...
\begin{itemize}\scriptsize
  \item Data source: Rice Data Portal
...
\begin{itemize}\scriptsize
  ...
  \item Query patterns and API access via \texttt{.env} token
  \item Claude Code fetches data directly---no web UI needed
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{itemize}\small
  ...
  \item Table schemas (SEP, DAILY, SF1, TICKERS)
  \item SQL query patterns (end-of-month, fundamentals)
  \item Precomputed variable checker
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{itemize}
  \item Data source (e.g., Rice Data Portal)
  \item Estimation window (trailing 60 months)
