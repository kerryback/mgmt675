...
\title{Module 8: Working with Financial Documents}
\author{Kerry Back}
\date{}
...
\begin{frame}[shrink=25]{Three Ways to Give AI Knowledge}
...
\begin{frame}[shrink=5]{RAG: Key Concepts}
\begin{columns}[T]
...
\begin{frame}{Under the Hood: Building a RAG Pipeline}
For those who want to understand the internals, ask Claude Code to build a RAG pipeline step by step.
\vspace{0.3cm}
\begin{enumerate}
  \item Install libraries (\texttt{langchain}, \texttt{chromadb}, an embedding model)
  \item Load and chunk a PDF, embed and store in a vector database
  \item Query the pipeline with finance-specific questions
\end{enumerate}
\end{frame}
...
\begin{enumerate}
  \item Ask Claude Code to build a RAG pipeline that loads a corporate annual report (e.g., Apple 10-K)
  \item Have it chunk and embed the document into a vector database
  \item Ask 5 finance-specific questions and evaluate whether answers are grounded or hallucinated
  \item Submit: code + evaluation
\end{enumerate}
\end{frame}
...
\begin{enumerate}
  ...
  \begin{itemize}\small
    ...
    \item Management guidance and accounting policies
  \end{itemize}
