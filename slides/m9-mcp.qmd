...
flowchart LR
  ...
  A <-->|"tool call"| M["<b>MCP<br>Server</b>"]
  M <-->|"executes"| T["<b>External<br>Tool</b>"]
  ...
  style M fill:#fed7aa,stroke:#ea580c,stroke-width:2px,color:#0f172a,font-size:28px,padding:24px
  style T fill:#f0fdf4,stroke:#22c55e,stroke-width:2px,color:#0f172a,font-size:28px,padding:24px
```

- The **LLM** decides what to do and formats tool calls
- The **MCP server** executes the call --- the LLM never touches the API directly
- Results flow back through the agent to the user
