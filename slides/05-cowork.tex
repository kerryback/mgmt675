...
\begin{itemize}\small
  \item Linux with Python and common libraries pre-installed
  \item Can \texttt{pip install} additional packages
  \item Your files from the selected folder (mounted into the VM)
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}\small
  \item Created on demand; destroyed when the session ends
  \item Isolated from your real files and applications
...
\begin{itemize}\scriptsize
  \item Run Python, R, shell scripts; \texttt{pip install} packages
...
\begin{itemize}\scriptsize
  \item \alert{No internet access}---can't call APIs or fetch live data
