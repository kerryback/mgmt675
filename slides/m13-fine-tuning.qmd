...
![](images/cosine-similarity.png){width=85%}
...
In LLMs, words and sentences are represented as high-dimensional vectors ([embeddings]{.amber}). Cosine similarity measures how related two meanings are:

- "profit" and "earnings" → high cosine similarity (similar direction)
- "profit" and "banana" → low cosine similarity (different directions)

:::{.explainer}
Fine-tuning adjusts these embedding vectors so that domain-specific relationships are captured. For example, after fine-tuning on financial text, "liability" moves [closer to]{.amber} neutral accounting terms and [farther from]{.amber} negative-sentiment words.
:::
...
$$h = b + w_1 x_1 + w_2 x_2 + w_3 x_3$$

The neuron "fires" (produces a positive output) only if this sum is positive:
...
graph TB
  ...
  style Layer_k fill:transparent,stroke:#f59e0b,color:#fff
  style Layer_k1 fill:transparent,stroke:#f59e0b,color:#fff
```
...
graph TB
  ...
  style B6 fill:#f59e0b,stroke:#f59e0b
  style Layer_k fill:transparent,stroke:#f59e0b,color:#fff
  style Adapter fill:transparent,stroke:#f59e0b,color:#fff
  style Layer_k1 fill:transparent,stroke:#f59e0b,color:#fff
```
