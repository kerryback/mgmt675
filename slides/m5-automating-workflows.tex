...
\vspace{0.3cm}
\begin{itemize}\small
  \item \textbf{Database Query}: Pull data via SQL, API, or MCP
  \item \textbf{Transform}: Calculate KPIs, variances, growth rates
  \item \textbf{Charts}: Generate visualizations with matplotlib or plotly
  \item \textbf{Narrative}: AI writes the commentary (``Revenue grew 8\% driven by\ldots'')
  \item \textbf{PowerPoint}: Assemble slides programmatically
\end{itemize}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{What the Pipeline Produces}]
\begin{enumerate}\small
  \item Title slide with date and department
  \item KPI summary table (revenue, margin, headcount)
  \item Trend charts (monthly revenue, YoY growth)
...
\end{column}
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Time Comparison}]
\begin{itemize}\small
