...
\title{Module 8: Verification and Governance}
\author{Kerry Back}
\date{}
...
Take analysis from Modules 1--7 and test whether it reproduces.
\vspace{0.3cm}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The exercise}
\begin{enumerate}\small
  \item Pick a financial analysis from Module 3 (DCF or portfolio optimization)
...
\end{columns}
\vspace{0.3cm}
If the numbers don't match, that's not a failure --- it's \alert{exactly why we verify}. And it's exactly why \textbf{skills} (Module 5) matter.
\end{frame}
...
Apply red-teaming protocols to the DCF models built in Module 3.
\vspace{0.2cm}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
...
\begin{enumerate}\small
  ...
  \item Feed a company with negative free cash flow --- is terminal value still positive?
  \item Compare AI's DCF output to a hand-built Excel model
\end{enumerate}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{itemize}\small
  ...
  \item How to improve the skill's guardrails
  \item Whether the verification checklist catches the errors
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{itemize}\small
  \item \textbf{Timestamp}: when the query ran
  \item \textbf{User}: who asked
  \item \textbf{Prompt}: the question
  \item \textbf{Response}: the AI's answer
  \item \textbf{Token count}: cost tracking
  \item \textbf{Model}: which LLM was used
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{itemize}\small
  ...
  \item \textbf{Cost control}: Track token usage by team or project
  \item \textbf{Improvement}: Identify common failures for skill updates
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
\begin{enumerate}
  \item \textbf{RAG citations}: Ground answers in source documents with traceable references (Module 6). AI answers based on \alert{your content}, not its training data.
...
\begin{enumerate}
  ...
  \begin{itemize}\small
    \item Skills for recurring workflows (Module 5)
    \item MCP integrations and Streamlit apps (Modules 4, 7)
  \end{itemize}
