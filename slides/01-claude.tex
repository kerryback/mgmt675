...
\node[mystep, below=of s4] (s5) {5. Claude Desktop --- Code};
\node[mystep, below=of s5] (s6) {6. Claude Code in VS Code};
...
\node[right=0.6cm of s6, text=titlegray, font=\scriptsize\itshape, text width=5.5cm] {AI coding inside an IDE};
\end{tikzpicture}
\end{center}
...
\textbf{Example:} \textit{``Create an Excel file to illustrate two-stage DCF analysis.''}
...
\textbf{Publish:} Publish the artifacts and share the links in Canvas chat.
\end{frame}
...
% ============================================================
\section{Code Execution Capabilities}

\begin{frame}{Code Execution: Sandbox vs.\ Local}

All chatbot code execution runs in a \textbf{sandbox}---an isolated container with restricted network access.
...
\toprule
\textbf{Mode} & \texttt{pip install} & \textbf{Internet} & \textbf{External APIs} \\
\midrule
ChatGPT (Data Analysis) & {\color{accentblue}\textbf{Yes}}\textsuperscript{*} & {\color{alertorange}\textbf{No}} & {\color{alertorange}\textbf{No}} \\
Claude Chat (web) & {\color{accentblue}\textbf{Yes}} & {\color{alertorange}\textbf{No}} & {\color{alertorange}\textbf{No}} \\
Claude Cowork (VM) & {\color{accentblue}\textbf{Yes}} & {\color{alertorange}\textbf{No}} & {\color{alertorange}\textbf{No}} \\
Claude Code (local) & {\color{accentblue}\textbf{Yes}} & {\color{accentblue}\textbf{Yes}} & {\color{accentblue}\textbf{Yes}} \\
\bottomrule
\end{tabular}
\end{center}

{\scriptsize \textsuperscript{*}ChatGPT added \texttt{pip install} in January 2026; previously limited to pre-installed packages.}

\vspace{0.2cm}

\textbf{Key insight:} Only \alert{Claude Code} (running on your machine) can call external APIs---Yahoo Finance, Alpha Vantage, Financial Modeling Prep, databases, etc.

\end{frame}

\begin{frame}[fragile]{Example: Live Stock Data with \texttt{yfinance}}

With Claude Code running locally, you can fetch \textbf{live financial data}:

\textbf{Prompt to Claude Code:}
\textit{``Get the last year of daily prices for AAPL and MSFT using yfinance. Plot the cumulative returns.''}

\textbf{Python code Claude writes and executes:}
\begin{lstlisting}
...
\texttt{yfinance} is free and requires no API key.  This \alert{fails in Chat, Cowork, and ChatGPT}---the sandbox blocks the network call.

\end{frame}

\begin{frame}[fragile]{Example: Financial Modeling Prep API}

APIs requiring \textbf{authentication} also work in Claude Code:

\textbf{Prompt to Claude Code:}
\textit{``Use Financial Modeling Prep to get Apple's income statement. My API key is in the environment variable FMP\_API\_KEY.''}
...
Free tier: 250 requests/day at \texttt{financialmodelingprep.com}.
...
\textbf{What It Does}
\begin{itemize}\small
  \item Sets the chatbot's persona and expertise
  \item Specifies response style and domain knowledge
  \item Sets guardrails and rules
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\textbf{How It Works}
...
\begin{itemize}
  \item Claude `skills' are text files (instructions) that the chatbot sends to the LLM when needed, triggered by matching circumstances
  \item The chatbot sends system prompt + prompt + chat history + skill, and the LLM responds accordingly
  \item This pattern is universal: Custom GPTs, Copilot instructions, \texttt{.cursorrules}, Gems---a skill is just a system prompt extension
\end{itemize}
\end{frame}
...
\end{columns}
\vspace{0.3cm}
\textbf{Ask the AI What It Can Do}
  \begin{itemize}
...
\end{columns}
\vspace{0.3cm}
\textbf{Example: What Does a Typical Query Cost via API?}\\
