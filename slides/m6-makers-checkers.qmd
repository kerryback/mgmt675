...
- Example: forecast sales growth, then derive everything else in relation to salesca
:::
...
Change the inputs and re-run. Notice how **ΔX / Sales** converges to the sales growth rate once growth stabilizes.

```{pyodide-python}
import pandas as pd

# ── Change these and click Run ───────────────────────────────
sales_0 = 100
growth  = [0.05, 0.10, 0.08, 0.03, 0.03]   # sales growth, years 1–5
ratio   = 2.0                                # X as a fixed multiple of Sales
# ────────────────────────────────────────────────────────────

sales   = [sales_0]
for g in growth:
    sales.append(sales[-1] * (1 + g))

x       = [s * ratio for s in sales]
delta_x = [x[i] - x[i-1] for i in range(1, 6)]
dx_pct  = [dx / s for dx, s in zip(delta_x, sales[1:])]

def pct(v): return f"{v:.0%}"
def num(v): return f"{v:,.1f}"

rows = {
    "Sales growth rate": [""] + [pct(g) for g in growth],
    "Sales":             [num(s) for s in sales],
    "X ratio to Sales":  [f"{ratio:.1f}"] * 6,
    "X":                 [num(v) for v in x],
    "ΔX":               [""] + [num(v) for v in delta_x],
    "ΔX / Sales":       [""] + [pct(v) for v in dx_pct],
}

df = pd.DataFrame(rows, index=pd.RangeIndex(6, name="Year")).T
INPUT_ROWS = ["Sales growth rate", "X ratio to Sales"]

styled = (df.style
    .apply(lambda row: (
        ["background-color:#dce6f1"] * 6 if row.name in INPUT_ROWS
        else ["background-color:#ffffff"] * 6), axis=1)
    .set_table_styles([
        {"selector": "th",
         "props": [("background-color","#00131f"), ("color","white"),
