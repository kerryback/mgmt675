...
\begin{baritemize}
  \item Download the file to your \textbf{Google Drive} (e.g., to My Drive or a subfolder)
  \item In your Colab notebook, ask Gemini to \textbf{mount your Google Drive}
  \item Gemini will generate code like:
    \begin{itemize}
      \item \texttt{from google.colab import drive}
      \item \texttt{drive.mount('/content/drive')}
    \end{itemize}
  \item Run the cell and authorize access when prompted
  \item Your Drive files are now at \texttt{/content/drive/MyDrive/...}
  \item Ask Gemini to load the file with pandas --- it will use the correct path and reader (\texttt{read\_excel}, \texttt{read\_csv}, etc.)
\end{baritemize}
