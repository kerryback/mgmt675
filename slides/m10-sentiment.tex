...
\title{Module 10: AI in Trading and Markets}
\author{Kerry Back}
\date{}
...
\begin{frame}{Multi-Agent Trading Systems}
\textbf{MarketSenseAI 2.0} (2025): GPT-4 + RAG multi-agent system.
\begin{itemize}\footnotesize
...
\begin{frame}[shrink=10]{The Cascade Architecture}
\begin{center}
\begin{tikzpicture}[
...
\node[sbox, minimum width=2.2cm] (headlines) {All\\Headlines};
\node[sbox, right=1cm of headlines, fill=accentblue!15] (finbert) {FinBERT\\{\tiny\normalfont\textasciitilde5ms}};

\node[sbox, right=1.5cm of finbert, yshift=0.8cm, fill=accentblue!8] (fast) {Immediate\\Signal};
\node[sbox, right=1.5cm of finbert, yshift=-0.8cm, fill=alertorange!15] (gpt) {GPT-4\\{\tiny\normalfont\textasciitilde1--5s}};
\node[sbox, right=1cm of gpt] (slow) {Enhanced\\Signal};

\draw[sarrow] (headlines) -- (finbert);
\draw[sarrow] (finbert) -- (fast) node[midway, above, font=\scriptsize\itshape, text=accentblue, sloped] {high conf.};
\draw[sarrow] (finbert) -- (gpt) node[midway, below, font=\scriptsize\itshape, text=alertorange, sloped] {low conf.};
\draw[sarrow] (gpt) -- (slow);
\end{tikzpicture}
\end{center}
\vspace{0.1cm}
\begin{columns}[T]
...
\begin{frame}[shrink=5]{The Propagation Problem}
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  sbox/.style={draw=titlegray, rounded corners, minimum width=1.6cm, minimum height=0.6cm, fill=excelinput, text=titlegray, font=\scriptsize\bfseries, align=center},
  sarrow/.style={-{Stealth[length=2.5mm]}, thick, draw=accentblue}
]
\node[sbox, fill=alertorange!20] (event) {Conflict};
\node[sbox, right=1cm of event] (oil) {Oil\\prices $\uparrow$};
\node[sbox, above right=0.3cm and 1cm of oil] (energy) {Energy\\stocks $\uparrow$};
\node[sbox, right=1cm of oil] (fert) {Fertilizer\\costs $\uparrow$};
\node[sbox, right=1cm of fert] (food) {Food\\prices $\uparrow$};
\node[sbox, below right=0.3cm and 1cm of food] (staples) {Staples\\margins $\downarrow$};

\draw[sarrow] (event) -- (oil);
\draw[sarrow] (oil) -- (energy);
\draw[sarrow] (oil) -- (fert);
\draw[sarrow] (fert) -- (food);
\draw[sarrow] (food) -- (staples);

\node[below=0.15cm of oil, font=\scriptsize\itshape, text=accentblue] {1st order};
\node[below=0.15cm of fert, font=\scriptsize\itshape, text=accentblue] {2nd order};
\node[below=0.15cm of food, font=\scriptsize\itshape, text=accentblue] {3rd order};
\end{tikzpicture}
\end{center}
\vspace{0.1cm}
\begin{itemize}
...
\begin{tabular}{@{} l l l @{}}
\toprule
\textbf{Event} & \textbf{What Happened} & \textbf{Impact} \\
\midrule
...
\begin{enumerate}
  \item Collect 20 recent financial headlines and have Claude classify sentiment, magnitude, and relevance for each.
  \item Compare classifications to actual stock price movements over the following day and calculate accuracy.
  \item Submit: spreadsheet with headlines, classifications, prices, and accuracy.
\end{enumerate}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item Lopez-Lira \& Tang (2023). Can ChatGPT Forecast Stock Price Movements? \textit{Journal of Finance}.
  \item Kirtac \& Germano (2024). Sentiment Trading with Large Language Models. \textit{Finance Research Letters}.
  \item Kim, Muhn \& Nikolaev (2024). Financial Statement Analysis with Large Language Models.
  \item Araci (2019). FinBERT: Financial Sentiment Analysis with Pre-Trained Language Models.
  \item Wu et al.\ (2023). BloombergGPT: A Large Language Model for Finance.
  \item Yang et al.\ (2023). FinGPT: Open-Source Financial Large Language Models.
  \item FinLlama (2024). Financial Sentiment Analysis for Algorithmic Trading. ICAIF '24.
  \item Caldara \& Iacoviello (2022). Measuring Geopolitical Risk. \textit{AER}, 112(4).
  \item Li et al.\ (2024). FinDKG: Dynamic Knowledge Graphs with LLMs. ICAIF '24.
\end{itemize}
}
\end{frame}
